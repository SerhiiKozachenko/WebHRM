...
{
    public class LessThanFilter<T> : BaseFilter<T>
    {
        ...
        public override IQueryable<T> Filter(string field, string value, IQueryable<T> query)
        {
            ...
            if (typeof(T).GetProperty(field).PropertyType == typeof(DateTime))
            {
                ...
                binaryExpression = Expression.LessThan(Expression.PropertyOrField(memberExpression, "Date"), searchExpression);
            }
            else
            {
                ...
                binaryExpression = Expression.LessThan(memberExpression, searchExpression);
            }
