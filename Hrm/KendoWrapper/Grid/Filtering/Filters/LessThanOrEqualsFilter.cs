...
{
    public class LessThanOrEqualsFilter<T> : BaseFilter<T>
    {
        ...
        public override IQueryable<T> Filter(string field, string value, IQueryable<T> query)
        {
            ...
            if (typeof(T).GetProperty(field).PropertyType == typeof(DateTime))
            {
                ...
                binaryExpression = Expression.LessThanOrEqual(Expression.PropertyOrField(memberExpression, "Date"), searchExpression);
            }
            else
            {
                ...
                binaryExpression = Expression.LessThanOrEqual(memberExpression, searchExpression);
            }
