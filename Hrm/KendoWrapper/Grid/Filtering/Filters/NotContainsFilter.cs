...
{
    public class NotContainsFilter<T> : BaseFilter<T>
    {
        ...
        public override IQueryable<T> Filter(string field, string value, IQueryable<T> query)
        {
            ...
            var lambda = Expression.Lambda<Func<T, bool>>(notContains, new[] { base.GetParameterExpression(memberExpression.Expression) });
